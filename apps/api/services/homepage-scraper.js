...
/**
 * Scrape homepage text content (excluding header and footer).
 * @param {string} url - The homepage URL
 * @returns {{ sections, stats, testimonials, fullText }}
 */
export async function scrapeHomepageContent(url) {
  ...
  try {
    ...
    // Scrape content after removing header/footer
    const result = await page.evaluate(() => {
      // Remove header, footer, nav, cookie banners, chat widgets
      ...
      // Fallback: if no sections found, use all headings
      ...
      // Extract stats/metrics (numbers with context)
...
/**
 * Compare two homepage snapshots and detect changes.
 */
